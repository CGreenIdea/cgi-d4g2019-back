...
public class ConsumptionService {
    ...
    public List<ConsumptionEntity> findAllById(String id) {
        return entityManager.createQuery("SELECT c FROM ConsumptionEntity c WHERE c.home_id = :id ORDER BY c.readingDate", ConsumptionEntity.class)
            .setParameter("id", id)
            .getResultList();
    }

    public List<ConsumptionEntity> getRangeConsumptionById(String id, String dateStart, String dateEnd) {
        return entityManager.createQuery("SELECT c FROM ConsumptionEntity c WHERE c.home_id = :id AND c.readingDate BETWEEN :dateStart AND :dateEnd", ConsumptionEntity.class)
            .setParameter("id", id)
